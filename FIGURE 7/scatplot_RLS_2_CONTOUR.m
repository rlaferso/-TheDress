function out = scatplot_RLS_2_CONTOUR(x,y,method,radius,N,n,po,ms,CONT)
...
switch po
    ...
    case {3,4}
        if po>3
            [c,h] = contour(xi,yi,zi,[CONT, CONT]); %  RLS [.14, .14]
            out.c = c;
        end %if
        hs = gsp(x,y,dd,ms);
        out.hs = hs;
...
switch method %Calculate Data Density
    ...
    case 'ci'
        ...
        area = pi*r^2;
        %dd = dd/area; %(this is the original line) and it = points/area; the vector is same length as input data  
        dd = dd;%/area; %RLS tried this to have bar represent counts
dd=dd./mean(dd);
dd = (dd./sum(dd))*100;  % percent of subjects
